...
class GNNData:
    ...
    def split_data(self, task: str, **kwargs):
        ...
        if task == 'link_prediction':
            transform = T.RandomLinkSplit(is_undirected=True, add_negative_train_samples=False, **kwargs)
            transform_dataset = T.Compose([transform, T.ToSparseTensor(remove_edge_index=False)])
            train_data, val_data, test_data = transform_dataset(self.data)
