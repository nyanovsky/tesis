...
sage_walk_df.to_csv(f"results/{version}/sage_walk_df.csv", index=False)

#%%
# %%
# continue walk here each time:
sage_walk_df = pd.read_csv(f"results/{version}/sage_walk_df.csv") 
sage_walk_df = hp_opt.walk(param_grid, sage_walk_df, 100, 1000, SAGEConv, sage_keys, dataset)
sage_walk_df.to_csv(f"results/{version}/sage_walk_df.csv", index=False)